...
class Comprova():
	...
	def saving(self,dict_data):
		...
		year = date[0]
		month = date[1]
		file_path = 'COLETORES/IMPLEMENTADOS/CHECAGENS/COMPROVA/COLETA/comprova_'+year+'_'+month+'.txt'		
	...
	def scrapper(self,url):
		...
		# Clicar no botão 'ver mais' pra abrir toda noticia.
		...
		infos['categories'] = [wrapper.find_element(*self.category_locator).text]
		...
		file_path = 'COLETORES/IMPLEMENTADOS/CHECAGENS/COMPROVA/HTML/'+value+'.html'
		with open(file_path,'w') as file:
			file.write(driver.page_source)
		infos["raw_file_name"] = value

		try:
			infos['image_link'] = main_txt.find_element(*self.image_locator).get_attribute('src')
		except NoSuchElementException:
			infos['image_link'] = 'None'
		driver.close()
		return infos